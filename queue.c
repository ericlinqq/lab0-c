...
static inline int remove_nodes(struct list_head *head, bool descend)
{
    int size = 1;
    // cppcheck-suppress nullPointer
    element_t *cur = list_last_entry(head, element_t, list);
    while (&cur->list != head->next) {
        // cppcheck-suppress nullPointer
        element_t *prev_ele = list_entry(cur->list.prev, element_t, list);
        int cmp = descend ? -strcmp(prev_ele->value, cur->value)
                          : strcmp(prev_ele->value, cur->value);
        if (cmp > 0) {
...
int q_merge(struct list_head *head, bool descend)
{
    ...
    queue_contex_t *first_entry = NULL, *last_entry = NULL;
    while (last != head->next) {
        if (first == last || first->prev == last)
            first = head->next;
        // cppcheck-suppress nullPointer
        first_entry = list_entry(first, queue_contex_t, chain);
        // cppcheck-suppress nullPointer
        last_entry = list_entry(last, queue_contex_t, chain);
        merge_two_lists(first_entry->q, last_entry->q, descend);
        first_entry->size += last_entry->size;
        first = first->next;
        last = last->prev;
    }
    // cppcheck-suppress nullPointer
